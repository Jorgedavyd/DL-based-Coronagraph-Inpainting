...
class InpaintingLoss(nn.Module):
    ...
    """
    def __init__(self, alpha: Iterable = [6, 1, 0.05, 120, 120]) -> None:
        assert (len(alpha) == 5), 'Must be 5 factor parameters for: L_holes, L_valid, L_percerptual, L_style_out, L_style_comp'
        super().__init__()
    ...
    def forward(self, I_out: Tensor, I_gt: Tensor, M_l_1: Tensor, M_l_2: Tensor) -> Tensor:
        ...
        mathcal_M: Tensor = M_l_1 ^ M_l_2

        # Per pixel loss
        L_outter: Tensor = torch.norm(~M_l_2.bool()*diff, p = 1)
        L_inner: Tensor = torch.norm(M_l_2*diff, p = 1)
        L_diff: Tensor = torch.norm(mathcal_M*diff, p = 1)
        L_pixel: Tensor = N_I_gt * (self.alpha[0]*L_outter + self.alpha[1]*L_inner + self.alpha[2]*L_diff)

        # Extracting features for the perceptual and style losses 
        psi_out: List[Tensor] = self.fe(I_out)
        psi_gt: List[Tensor] = self.fe(I_gt)

        psi_masked_out: List[Tensor] = self.fe(I_out*M_l_2)
        psi_masked_gt: List[Tensor] = self.fe(I_gt*M_l_2)

        L_perceptual: Tensor = self.alpha[3] * (Tensor([torch.norm(phi_out-phi_gt, p = 1) + torch.norm(psi_out_m - psi_gt_m, p = 1)\
                                         for phi_out, phi_gt, psi_out_m, psi_gt_m in zip(psi_out, psi_gt, psi_masked_out, psi_masked_gt)])\
                                            / self.N_phi_p).sum()
        
        #Style loss 
        ## Changing size of the features from the images
        change_dim: Callable[[List[Tensor]], List[Tensor]] = lambda P: [tensor.view(tensor.shape[0], tensor.shape[1], -1) for tensor in P]
        ...
        L_n: Callable[[List[Tensor], List[Tensor]], List[Tensor]] = lambda out_list, gt_list: Tensor([torch.norm(out@out.tranpose(-2, -1) - gt@gt.tranpose(-2, -1), p = 1)\
                                                for out, gt in zip(out_list, gt_list)])

        L_style: Tensor =  ((self.alpha[4]*L_n(psi_out, psi_gt) + self.alpha[5]*L_n(psi_masked_out, psi_masked_gt) \
                    + self.alpha[6]*L_n(psi_mathcal_M_out, psi_mathcal_M_gt))/self.F_p).sum()
