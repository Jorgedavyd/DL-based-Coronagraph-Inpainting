...
class SmallUNet(TrainingPhase):
    ...
    def compute_train_loss(self, batch: Tensor) -> Tuple[None, Tensor]:
        ...
        for layer in range(self.layers):
            I_out, updated_mask = self._single_forward(
                gt, prior_mask, layer
            )  # Feeding the ground truth so the training is not dependent of prior layers
    ...
    def compute_val_metrics(self, batch: Tensor) -> Tuple[Dict[str, float]]:
        ...
        metrics = {
            key: value
            for key, value in zip(self.criterion.labels, [arg.item() for arg in args])
        }
        return metrics
...
class CrossModel(SmallUNet):
    ...
    def _single_forward(
        self, x_1: Tensor, x_2: Tensor, time: Tensor, mask_in: Tensor, layer: int
    ) -> Tuple[Tensor, Tensor]:
        b, _, _, _ = x_2.shape
        gt = x_2.clone()
        ...
        f_5_scaled, f_4_scaled, f_3_scaled, f_2_scaled, f_1_scaled, f_scaled, _ = (
            self.encoder(x_1, torch.ones_like(x_1), layer)
        )
    ...
    def forward(
        self, x_1: Tensor, x_2: Tensor, time: float, mask_in: Tensor
    ) -> Tuple[Tensor, Tensor]:
...
from fourier import _FourierConv, ResidualFourierConvTranspose2d
from torch.fft import fftn, ifftn
from icecream import ic


class FourierVAE(TrainingPhase):
    def __init__(self, name_run: str, criterion) -> None:
        super().__init__(name_run, criterion)
        self.fft = fftn
        self.ifft = ifftn
    ...
    def forward(self, x: Tensor, mask_in: Tensor) -> Tensor:
        ...
        out = self.reparametrization(out, 1024)  # -> 1024, 32, 32
